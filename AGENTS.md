...
- **M5 partial (PR #4)** — stereo-pair linking (default on, app-bar toggle; gain/mute/solo/EQ mirror, pans inverted) and monitor-feed auto-exclusion on fresh sessions (In Ear/Phones/IEM/Talkback/Line Out/Monitor; `*_Out` stems stay in). **Remaining:** A/B mix snapshots, batch export queue, per-pair unlink, signed releases (needs Apple Developer account — user decision).
