...
- **M4 Android (code complete, NEEDS DEVICE TEST)** — SAF end-to-end: Kotlin MethodChannel `durecmix/saf` (pick/create/openFd/displayName, persistable read permission), engine `InputHandle`/`OutputHandle` (path or raw fd; one fresh fd per engine call, ownership moves to Rust), bridge fd params on load/analyze/play/render. Fixes the v0.3.0 crash cause (file_selector copied the multi-GB file). Still open: on-device verification (`adb logcat` if it misbehaves).
