...
- **M3b next** — FLAC/MP3 export, trim/fade handles, BPM detection, filename templating (match the Python tool's `{name}_{peak}_{bpm}BPM_{timestamp}` pattern), loudness-report file. Then M4 (mobile) per `docs/PLAN.md`.
