...
- **M5 remaining** — stereo-pair linking (engine `stereo_pair_base()` exists), A/B snapshots, channel-name presets (auto-exclude monitor buses), batch queue, signed releases (needs Apple Developer account).
