...
- **Multi-file export** — the browser's selection mode (checklist icon) pre-ticks multichannel takes, lets you edit each output name (extension follows the chosen format), and renders everything with the current mix (mapped by track name) into a `Mixdown/` subfolder — then share straight to Nextcloud/Drive/WhatsApp from the result bar (Android). The plain list stays a simple "tap to open" file picker.
